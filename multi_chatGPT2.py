...
database_name = 'chatdb'       # データベース名    
container_name = 'chathistory'  # コンテナー名    
...
def main():    
    ...
    if hasattr(st, 'experimental_user'):  
        ...
        if user and 'oid' in user:  
            user_id = user['oid']  # ユーザーのメールアドレスを使用  
    ...
    with st.sidebar:    
        ...
        past_message_count = st.slider("過去メッセージの数", min_value=1, max_value=50, value=10)  # 最大値を50に変更    
        reasoning_effort = st.selectbox("Reasoning Effort", options=["low", "medium", "high"], index=1)    
        st.header("検索設定")    
        topNDocuments = st.slider("取得するドキュメント数", min_value=1, max_value=10, value=5)    
        strictness = st.slider("厳密度 (スコアの閾値)", min_value=0.0, max_value=10.0, value=0.1, step=0.1)  # 最大値を10に変更    
    ...
    if prompt := st.chat_input("ご質問を入力してください:"):    
        ...
        # ルールメッセージを追加    
        rule_message = ("回答する際は、以下のルールに従ってください：\n"    
                        "1. 簡潔かつ正確に回答してください。\n"    
        ...
        try:    
            # モデルの最大トークン数（モデルによって異なります）    
            max_tokens = 200000    
            # モデルの選択    
            model_to_use = "o1" if st.session_state.images else "o3-mini"    
